...
import type { SortKey, Document } from "@/lib/library/types";
...
export default function LibraryPage() {
  ...
  const filtered = useMemo(() => {
    let docs = DOCUMENTS.filter((doc) => {
      if (activeSort === "saved" && !savedDocs.includes(doc.id)) {
        return false;
      }
      if (activeType !== "Tất cả" && doc.type !== activeType) return false;
      if (activeSubject !== "all" && doc.subjectId !== activeSubject)
        return false;
      if (query) {
    ...
    if (activeSort === "mostDownloaded") {
  docs = [...docs].sort((a, b) => b.downloadsNum - a.downloadsNum);
}
...
if (activeSort === "oldest") {
  docs = [...docs].sort((a, b) => a.id - b.id);
    ...
    return docs;
  }, [activeType, activeSubject, activeSort, query, savedDocs]);
